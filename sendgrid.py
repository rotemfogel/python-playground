...
class SendGridOperator:
    ...
    def execute(self):
        ...
        end_time = self._execution_date.add(minutes=4).add(seconds=59).format('%Y-%m-%dT%H:%M:%SZ')
        params = {'query': 'last_event_time BETWEEN TIMESTAMP "{}" AND TIMESTAMP "{}"'.format(start_time, end_time)}
        query_params = '&'.join(
            list(map(lambda kv: ("{}={}".format(kv[0], parse.quote(kv[1]))), params.items())))
                        ...
                        if messages:
                            with open(uri=uri, mode='wb') as s3_file:
                                ...
                                for message in messages:
                                    if message:
                                        s3_file.write((json.dumps(message) + '\n').encode())
                            print("Uploaded {} to S3".format(self._table))
                        else:
                            print('Received empty response for payload {}'.format(params))
                        ix = 3  # end loop
                else:
                    ix += 1
