...
class S3ToGlueOperator:
    ...
    def __init__(
        ...
        self.post_key_path = (
            post_key_path.strip("/") if post_key_path else post_key_path
        )
        self.database = database
        self._hook = boto3.Session(
    ...
    def execute(self):
        ...
        if self.post_key_path:
            uri = "s3://{bucket}/{key}/{post_key_path}/{file}.json.gz".format(
                bucket=self.bucket,
                key=self.key,
            ...
            uri = "s3://{bucket}/{key}/{file}.json.gz".format(
                bucket=self.bucket, key=self.key, file=self.file
            )
    ...
    def _get_table(self, table_name: str) -> Optional[TableDef]:
        try:
            table_def = self._hook.get_table(
    ...
    def _create_table(self, table_def: TableDef):
        ...
        try:
            self._hook.create_table(DatabaseName=self.database, TableInput=table_input)
    ...
    def _update_table(self, table_def: TableDef):
        table_input = table_def.to_glue(self.database)
        try:
            self._hook.update_table(DatabaseName=self.database, TableInput=table_input)
