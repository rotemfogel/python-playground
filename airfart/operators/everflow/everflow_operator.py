...
class EverFlowOperator(object):
    ...
    def execute(self):
        ...
        if everflow_records and len(everflow_records) > 0:
            ...
            load_dotenv()
            everflow_config: Dict[str, Any] = json.loads(
                os.getenv("everflow_api_config")
            )
            url = everflow_config["domain"]
            event_query_params = dict(
                verification_token=everflow_config["verification_token"],
            ...
            for record in everflow_records:
                ...
                if self.report_click:
                    ...
                    status_code = self._send_http(
                        f"{url}/sdk/click", register_query_params
                    )
                    if status_code != 200:
                ...
                if status_code == 200:
                    ...
                    if status_code != 200:
                        ...
                        self.log.info(
                            f"sent conversion callback for record {str(record)}"
                        )
            if failures:
                raise AirflowException(
                    f"operator failed to send {len(failures)} "
