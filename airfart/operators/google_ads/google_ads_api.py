...
from airfart.hooks.google_ads.google_ads_api import GoogleAdsApiType, GoogleAdsApiHook
...
class GoogleAdsApiOperator(BaseDataToS3Operator):
    ...
    def get_hook(self):
        if not self.__hook:
            self.__hook = GoogleAdsApiHook(
                api_type=self.method, account_id=self.account_id
            )
        return self.__hook
