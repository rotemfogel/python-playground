...
class GlueCatalogHook(LoggingMixin):
    ...
    def __chunk(self, what: List[object]) -> List[List[object]]:
        ...
        return [what[i * self._chunk_size:(i + 1) * self._chunk_size] for i in
                range((len(what) + self._chunk_size - 1) // self._chunk_size)]
    ...
    def get_table_versions(self, db: str, table: str) -> Optional[list]:
        try:
            ...
            return self._paginate('get_table_versions', 'TableVersions', 'VersionId', **args)
        except Exception as e:
            if 'EntityNotFoundException' in str(e):
                return None
            raise e
    ...
    def delete_tables(self, db: str, tables_to_remove: List[str]) -> None:
        ...
        for chunk in table_chunks:
            self.get_conn().batch_delete_table(
                DatabaseName=db,
                TablesToDelete=list(map(lambda x: str(x), chunk))
            )
        self.log.info(
            f'dropped {len(tables_to_remove)} tables from database {db}')

    def delete_table_versions(self, db: str, table: str) -> None:
        # convert results to int and sort
        versions = sorted(list(map(lambda x: int(x), self.get_table_versions(db, table))))
        if len(versions) > self._chunk_size:
            ...
            self.log.info(f'dropped {len(versions_to_drop)} versions out of {len(versions)} from table {db}.{table}')
    ...
    def delete_all_table_versions(self, db: str) -> None:
        ...
        if self.get_database(db):
            ...
            tables = list(filter(lambda x: not str(x).startswith('tmp_'), self.get_tables(db)))
            for table in tables:
    ...
    def get_table_input(self, database: str, table: str) -> dict:
        ...
        table_input = dict(Name=table_name, StorageDescriptor=table_result['StorageDescriptor'])
        # Required table input parameters
        table_input['StorageDescriptor']['SerdeInfo']['Name'] = \
            table_result.get('SerdeInfo', {}).get('Name', table_name)
        table_input["TableType"] = table_result.get('TableType', "EXTERNAL_TABLE")
        table_input['Parameters'] = table_result.get('Parameters', {
            'EXTERNAL': 'TRUE',
    ...
    def glue_add_columns(self, database, table, columns):
        ...
        try:
            ...
            self.log.info("Columns: {cols} added to the table: {db}.{tbl}".format(db=database, tbl=table, cols=columns))
    ...
    def get_glue_list_columns(self, database, table) -> list:
        ...
        for num, col in enumerate(list_glue_cols_raw):
            curr_list.append({**{'row_num': num}, **dict((k.strip(), v.strip()) for k, v in col.items())})
        return curr_list

    def create_table(self, database_name, table_input):
        result = self.get_conn().create_table(DatabaseName=database_name,
                                              TableInput=table_input)
        return result

    def update_table(self, database_name, table_input):
        result = self.get_conn().update_table(DatabaseName=database_name,
                                              TableInput=table_input)
        return result

    def get_table_def(self, database_name: str, table_name: str) -> Optional[TableDef]:
        try:
            table_def = self.get_table(database_name=database_name, table_name=table_name)
    ...
    def update_table_def(self, database_name, table_def: TableDef):
        table_input = table_def.to_glue(database_name)
        try:
            self.update_table(database_name=database_name,
