    ...
    while True:
        ...
        for page in pages:
            ...
            if results:
                queries = list(
                    filter(
                        lambda x: x["Status"]["State"]
                        in ["SUCCEEDED", "FAILED", "CANCELLED"]
                        and float(
                            x.get("Statistics", {DATA_SCANNED_IN_BYTES: 0}).get(
                        ...
                        if submission_time:
                            ...
                            if (
                                LOWER_TIME_LIMIT.diff(
                                    pendulum.instance(submission_time), abs=False
                                ).minutes
                                < 0
                            ):
                                break
                            ...
                            records.append(
                                {
                                    ...
                                    "database": query["QueryExecutionContext"][
                                        "Database"
                                    ],
                                    ...
                                    "planning_time_millis": statistics[
                                        "QueryPlanningTimeInMillis"
                                    ],
                                    "queue_time_millis": statistics[
