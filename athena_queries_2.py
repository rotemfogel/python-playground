...
if __name__ == "__main__":
    ...
    while True:
        ...
        results = athena_client.batch_get_query_execution(QueryExecutionIds=query_executions['QueryExecutionIds'])
        if results:
            # create a tuple of the query and the diff in minutes from LOWER_TIME_LIMIT
            query_times = list(map(lambda q:
                                   (q, LOWER_TIME_LIMIT.diff(
                                       pendulum.instance(q['Status']['SubmissionDateTime']), abs=False).minutes if q[
                                       'Status'].get('SubmissionDateTime') else -1)
            ...
            queries = list(map(lambda q: q[0],
                               list(filter(lambda x: x[0]['Status']['State'] in ['SUCCEEDED', 'FAILED', 'CANCELLED'] and
                                                     float(x[0].get('Statistics', {DATA_SCANNED_IN_BYTES: 0}).get(
                                                         DATA_SCANNED_IN_BYTES) / pow(1024, 3)) > MIN_GB_TO_LOG and x[
                                                         -1] > 0,
                                           query_times))))
            ...
            for query in queries:
                ...
                if query_execution_id:
                    ...
                    records.append({
                        ...
                        'submission_time': query['Status']['SubmissionDateTime'].timestamp(),
                        'bytes_scanned': bytes_scanned,
                        'execution_time_millis': statistics['EngineExecutionTimeInMillis'],
                        'planning_time_millis': statistics.get('QueryPlanningTimeInMillis', -1),
