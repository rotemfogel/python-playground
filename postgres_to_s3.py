...
class PostgresToS3Operator:
    ...
    def __init__(
        ...
        assert (
            self.output_format in self._allowed_formats
        ), f"output_format should be either {self.JSON}, {self.PARQUET} or {self.CSV}! "
    ...
    def execute(self):
        ...
        if self.output_format == self.PARQUET:
            ...
            if self.output_format == self.JSON:
                ...
                values = list(
                    map(lambda x: json.dumps(x), df.to_dict(orient="records"))
                )
            else:  # self.CSV
...
if __name__ == "__main__":
    for out_format in [PostgresToS3Operator.CSV, PostgresToS3Operator.JSON]:
        PostgresToS3Operator(
            sql="select * from catalog_updates",
