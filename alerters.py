...
alert_tasks: dict = {
    "dags": {
        ...
        "vertica_health_check": {
            "tasks": [
                {"all_nodes_up": {}},
                {"license_utilization": {"from": 5, "to": 21}},
            ]
        },
    },
...
def report_to_pagerduty_with_conf(
    ...
    try:
        if type(dag_tasks[0]) is dict:
            ...
            tasks_list = set(
                list(flat_map(lambda x: x.keys(), dag_tasks)) + default_tasks
            )
    ...
    should_report = (
        should_report and report_hours["from"] <= execution_hour <= report_hours["to"]
    )
...
if __name__ == "__main__":
    ...
    assert report_to_pagerduty_with_conf(
        dag_id="user_registration_trackings",
        task_id="copy_qa",
        execution_date=execution_date,
    )
    assert report_to_pagerduty_with_conf(
        dag_id="subscription_events", task_id="ccc", execution_date=execution_date
    )
    assert report_to_pagerduty_with_conf(
        dag_id="emr_page_events",
        task_id="partition_rollup_add_step",
        execution_date=execution_date,
    )
    assert report_to_pagerduty_with_conf(
        dag_id="vertica_health_check",
